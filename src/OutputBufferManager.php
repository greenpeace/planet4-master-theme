...
class OutputBufferManager
{
     ...
     *
     */
    public function manage_output_buffer ($buffer) {
        $dom = new DOMDocument();
    ...
    {
        $no_results = $xpath->query("//*[contains(@class, 'p4-query-loop')]//*[contains(@class, 'wp-block-query-no-results')]");
    ...
    {
        $links = $xpath->query("//a[contains(@href, '.pdf') and not(contains(@class, 'search-result-item-headline')) and not(contains(@class, 'cover-card-heading')) and not(contains(@class, 'pdf-link'))]");
    ...
    private function setup_external_links(DOMXPath $xpath): void
    {
        ...
        $containers = [".//*[contains(@class, 'page-content')]//a", ".//article//a", ".//*[contains(@class, 'author-details')]//a"];
