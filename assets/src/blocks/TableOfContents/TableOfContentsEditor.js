    ...
    <InspectorControls>
      <PanelBody title={__('Settings', 'planet4-master-theme-backend')}>
        <p className="components-base-control__help">
          {__('Choose the headings to be displayed in the table of contents.', 'planet4-master-theme-backend')}
        </p>
        ...
        <Button
          ...
          style={{marginRight: 5}}
        >
          {__('Add level', 'planet4-master-theme-backend')}
        </Button>
        <Button
          variant="secondary"
          onClick={removeLevel}
        ...
        >
          {__('Remove level', 'planet4-master-theme-backend')}
        </Button>
      </PanelBody>
    ...
    <section className={`block table-of-contents-block table-of-contents-${style} ${className ?? ''}`}>
      ...
      {menuItems.length > 0 ?
        ...
        <div className="EmptyMessage">
          {__('There are not any pre-established headings that this block can display in the form of a table of content. Please add headings to your page or choose another heading size.', 'planet4-master-theme-backend')}
        </div>
      }
    </section>
  );
