...
class P4_Custom_Taxonomy {
	...
	public function add_taxonomy_as_permalink_structure( $tags ) {
		$tags[ self::TAXONOMY_PARAMETER ] = __(
				'P4 page type (A p4 page type term.)',
	...
	public function save_taxonomy_page_type( $post_id, $post ) {
		...
		if (
				...
				&& filter_var(
						wp_unslash( $_POST[ self::TAXONOMY ] ),
						FILTER_VALIDATE_INT
				)
		) {
	...
	public function filter_posts_by_page_type( $post_type, $which ) {
		...
		// Display filter HTML.
		?>
		<select name="<?php echo esc_attr( self::TAXONOMY ); ?>" id="<?php echo esc_attr( self::TAXONOMY ); ?>"
				class="postform">
