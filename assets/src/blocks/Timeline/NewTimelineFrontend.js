...
export const NewTimelineFrontend = ({attributes}) => {
  ...
  const TimelineEvent = ({event}) => {
    ...
    return (
      <li className="timeline-block-event">
        ...
        >
          <span aria-hidden="true">{getLocalizedDate(event.Day, event.Month)}</span>
        </p>
  ...
  /* translators: 1: timeline title, 2: total items, 3: first date, 4: last date */
    __('%1$s, %2$d items from %3$s to %4$s.', 'planet4-master-theme-backend'),
    timeline_title,
    total,
    firstDate,
    lastDate
  ...
  return (
    <section className={`block timeline-block new-timeline-block ${className ?? ''}`} aria-label={summaryText}>
      ...
      {loading && <p className="text-center">{__('Loading…', 'planet4-master-theme-backend')}</p>}
