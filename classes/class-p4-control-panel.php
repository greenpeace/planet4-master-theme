...
class P4_Control_Panel {
	...
	public function add_items() {
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'editor' ) ) {
			...
			if (
			is_plugin_active(
				'planet4-plugin-gutenberg-engagingnetworks/planet4-gutenberg-engagingnetworks.php'
			)
			) {
				$this->add_item(
					[
					...
					[
						...
						'subitems' => [
							[
								...
								// If we give 'url' key instead of 'action' then we will do usual request instead of ajax request.
			...
			if ( wp_verify_nonce( $cp_nonce, 'cp-action' ) && 'check_engaging_networks' === $cp_action ) {
				...
				if ( isset( $main_settings['p4en_private_api'] ) && $main_settings['p4en_private_api'] ) {
					...
					if ( $ens_api->is_authenticated() ) {
						...
						$response['message'] = __(
							'Planet4 is not connected to EngagingNetworks. Please, make sure you have registered your IP address for the ENS API key you have supplied in plugin settings page.',
							'planet4-master-theme-backend'
						);
						$response['class']   = 'cp-error';
					}
	...
	public function check_elasticsearch() {
		...
		if ( wp_doing_ajax() ) {
			...
			if ( wp_verify_nonce( $cp_nonce, 'cp-action' ) && 'check_elasticsearch' === $cp_action ) {
				...
				if (
				( new ES() )->get_elasticsearch_version()
				) {   // For version up to 2.8.* we can call ep_elasticsearch_can_connect() to check connection.
					$response['message'] = __(
						'Planet4 is connected to Elasticsearch',
						'planet4-master-theme-backend'
					);
					$response['class']   = 'cp-success';
				} else {
					$response['message'] = __(
						'Planet4 is not connected to Elasticsearch',
						'planet4-master-theme-backend'
					);
					$response['class']   = 'cp-error';
				}
